...
/*------------------------------------------*/
    ...
    $sql="select question,opt1,opt2,opt3,opt4,ans from quiz where test_id='other' and sr_no='$sr';";
    $r=mysqli_query($conn,$sql);
    $rc=mysqli_num_rows($r);
    echo "<br>".$rc;    

    echo "<div id='f2'><form method='GET'  action='marks.php'>";
if($rc>0){
    ...
    while($row =mysqli_fetch_assoc($r)){
        ...
        <input class='btn btn-success' name='submit' id='next' type='submit' value='Submit' onclick='btn_next();'>
     ";
         exit();
    }   
echo "</form>";
...
$up=mysqli_query($conn,$sql);
    ...
    echo "
    <form action='del.php' method='POST'><p>Quiz completed</p>
    <button type='submit' name='complete' id='next' onclick='btn_next();' class='btn btn-danger'>Save and Exit</button>
    </form></div>
    ";
